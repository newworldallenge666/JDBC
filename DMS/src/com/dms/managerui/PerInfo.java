...
public class PerInfo extends JPanel {
	...
	private JCheckBox checkBoxStuMan;
	private JCheckBox checkBoxStuWoman;
	...
	public PerInfo(Manager m, String MANAGER_ID) {
		this.manager = m;
		...
		labDormInfo.setBounds(48, 201, 128, 41);
		add(labDormInfo);
		...
		butSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				UpdatePersonInfo updatePersonalInfoForm = new UpdatePersonInfo();
		...
		checkBoxStuMan = new JCheckBox("男");
		checkBoxStuMan.setFont(new Font("宋体", Font.PLAIN, 17));
		checkBoxStuMan.setBounds(163, 330, 60, 23);
		add(checkBoxStuMan);
		
		checkBoxStuWoman = new JCheckBox("女");
		checkBoxStuWoman.setFont(new Font("宋体", Font.PLAIN, 17));
		checkBoxStuWoman.setBounds(231, 330, 96, 23);
		add(checkBoxStuWoman);
	...
	public void showInfo() throws DataBaseException, QueryResultIsNullException{
		...
		txtDMID.setText(manager.getId());
		 ...
		 txtARemark.setText("周末门禁时间为23:50，平时门禁时间为23:00。");
		 if(dormitoryBuildingInfo.getStuSex().equals("女")){
			checkBoxStuWoman.setSelected(true); 
		 }else{
			 checkBoxStuMan.setSelected(true);
		 } 
	...
	private void Editable(boolean statue){
		...
		txtDMPhone.setEditable(statue);	  
		  ...
		  butSave.setEnabled(statue);
		  txtLiveNum.setEnabled(false); 
		  txtConNum.setEnabled(false);  
		  checkBoxStuMan.setEnabled(false);
		  checkBoxStuWoman.setEnabled(false);
